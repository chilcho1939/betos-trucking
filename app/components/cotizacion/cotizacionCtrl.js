myApp.controller('cotizacionCtrl', ['$scope', function ($scope) {
    ...
    $scope.cambiarCiudades = function(pais) {
        if(pais == 'México') {
    ...
    $scope.sendMail = function () {
        ...
        message += '<p style="font-size:18px;">Solicito un flete de <strong>' + $scope.cotizacionObject.detalle.producto + '</strong> en <strong>' + $scope.cotizacionObject.detalle.tipoTransporte + transfer() + '</strong> , necesito <strong>' + $scope.cotizacionObject.detalle.numeroViajes + '</strong> viaje(s) ';
